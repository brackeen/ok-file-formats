 ...
 *
 * The color_format parameter specifies the format to return the pixel data (RGBA or BGRA, with or 
 * without premultiplied alpha).
 *
 * If flip_y is true, the returned image data is flipped along the vertical axis, so that the first 
 * row of data is the last row in the image.
 */
...
/**
 * Inflates at most dst_length bytes. Returns number of bytes inflated, or a negative value if an 
 * error occured.
 */
